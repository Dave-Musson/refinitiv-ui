...
import { isHex, AccessibilityColor } from '@refinitiv-ui/utils/color.js';
...
export class ColorPicker extends ControlElement {
  ...
  protected update (changedProperties: PropertyValues): void {
    if (changedProperties.has(TranslatePropertyKey)) {
      void this.updateColorDescription();
    }
  ...
  private get selectionTemplate (): TemplateResult | null {
    ...
    role="status"
    aria-live="polite"
    aria-label="${this._colorDescription}"></div>`;
  }
   ...
   * @returns {void}
   */
  private async updateColorDescription (): Promise<void> {
    const { name, lightIntense, colorName, colorMixPercent, colorMixName } = new AccessibilityColor(this.value).toLocalized();
    if (name) {
      this._colorDescription = name;
      return;
    }
    const [
      lightIntenseT,
      colorNameT,
      colorMixPercentT,
      colorMixNameT] = await Promise.all([this.colorTPromise(lightIntense),
      this.colorTPromise(colorName),
      this.colorTPromise('WITH', { number: colorMixPercent }),
      this.colorTPromise(colorMixName)]);
    if (colorMixPercent) {
      this._colorDescription = `${lightIntenseT}${colorNameT} ${colorMixPercentT} ${colorMixNameT}`;
    }
    else {
      this._colorDescription = `${lightIntenseT}${colorNameT}`;
    }
  }
